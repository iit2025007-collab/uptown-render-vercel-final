...
const angleWords = ['front view', 'side view', 'detail shot', 'styled look'];

function imageUrl(product, colorName, index, angle = 0) {
  const query = encodeURIComponent(`${colorName} ${product.query} ${angleWords[angle]}`.replace(/\s+/g, ','));
  const lock = 2000 + (index * 7) + angle;
  return `https://loremflickr.com/900/1200/${query}?lock=${lock}`;
}
